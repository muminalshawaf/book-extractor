...
const AdminProcessing = () => {
  ...
  // AI Testing States
  const [aiTestResults, setAiTestResults] = useState<Record<string, { response: string; loading: boolean; error?: string }>>({
    'gemini-pro': { response: '', loading: false },
    'gemini-flash': { response: '', loading: false },
    'deepseek': { response: '', loading: false }
  });
  ...
  const testAIModel = async (modelType: 'gemini-pro' | 'gemini-flash' | 'deepseek') => {
    setAiTestResults(prev => ({
      ...prev,
      [modelType]: { ...prev[modelType], loading: true, error: undefined }
    }));
    ...
    try {
      ...
      switch (modelType) {
        case 'gemini-pro':
          // Test Gemini Pro 2.5 via summarize function (which uses Gemini Pro)
          result = await callFunction('summarize', {
            text: 'hello',
            lang: 'en',
            page: 1,
            title: 'Test'
        ...
        case 'gemini-flash':
          // Test Gemini Flash 2.5 via OCR function
          result = await callFunction('ocr-gemini', {
            imageUrl: 'https://via.placeholder.com/300x200/ffffff/000000?text=Hello',
            language: 'en'
          });
          break;
        
        case 'deepseek':
          // Test DeepSeek via QA function
          result = await callFunction('qa', {
            question: 'hello',
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-medium">Gemini Pro 2.5</h3>
                    <Button 
                      size="sm" 
                      onClick={() => testAIModel('gemini-pro')}
                      disabled={aiTestResults['gemini-pro'].loading}
                    >
                      {aiTestResults['gemini-pro'].loading ? (
                        <>
                  ...
                  <div className="text-sm space-y-2">
                    <div className="text-muted-foreground">Via: Summarize Function</div>
                    {aiTestResults['gemini-pro'].error && (
                      <Alert>
                    ...
                    {aiTestResults['gemini-pro'].response && (
                ...
                {/* Gemini Flash 2.5 Test */}
                <div className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-medium">Gemini Flash 2.5</h3>
                    <Button 
                      size="sm" 
                      onClick={() => testAIModel('gemini-flash')}
                      disabled={aiTestResults['gemini-flash'].loading}
                    >
                      {aiTestResults['gemini-flash'].loading ? (
                        <>
                          <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                          Testing...
                        </>
                  ...
                  <div className="text-sm space-y-2">
                    <div className="text-muted-foreground">Via: OCR Gemini Function</div>
                    {aiTestResults['gemini-flash'].error && (
                      <Alert>
                        <XCircle className="h-4 w-4" />
                        <AlertDescription className="text-red-600">
                          {aiTestResults['gemini-flash'].error}
                    ...
                    {aiTestResults['gemini-flash'].response && (
                      <div className="bg-muted p-2 rounded text-xs max-h-32 overflow-y-auto">
                        <pre className="whitespace-pre-wrap break-all">
                          {aiTestResults['gemini-flash'].response}
                        </pre>
                ...
                <div className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-medium">DeepSeek Reasoner</h3>
                    <Button 
                      size="sm" 
                      onClick={() => testAIModel('deepseek')}
                      disabled={aiTestResults['deepseek'].loading}
                    >
                      {aiTestResults['deepseek'].loading ? (
                        <>
                          <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                          Testing...
                        </>
                  ...
                  <div className="text-sm space-y-2">
                    <div className="text-muted-foreground">Via: QA Function</div>
                    {aiTestResults['deepseek'].error && (
                      <Alert>
                        <XCircle className="h-4 w-4" />
                        <AlertDescription className="text-red-600">
                          {aiTestResults['deepseek'].error}
                    ...
                    {aiTestResults['deepseek'].response && (
                      <div className="bg-muted p-2 rounded text-xs max-h-32 overflow-y-auto">
                        <pre className="whitespace-pre-wrap break-all">
                          {aiTestResults['deepseek'].response}
                        </pre>
